...
class AES:
    ...
    def __init__(self):
        self.regr = None
        print("AES model, training train_model")

    def get_regr_model(self):
        return self.regr

    def train_module(self):
        df = pd.read_csv('aes_data_trace.csv', header=None, names=['num_packets', 'payload_size', 'get_time', 'exec_time'])
        df.head()
        X = df[['num_packets', 'payload_size']].to_numpy()
        Y = df['exec_time'].to_numpy()
        (X_train, X_test, Y_train, Y_test) = train_test_split(X, Y)        
        self.regr = KNeighborsRegressor(n_neighbors=5)
    ...
    def predict(self, attr1, attr2):
        ...
        print("[AES Latency Prediction] Prediction : {0}".format(Y_predict[0][0]))
